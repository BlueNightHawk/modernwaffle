...
void CBSPRenderer::Init( void ) 
{
	...
	glFogCoordPointer				= (PFNGLFOGCOORDPOINTEREXTPROC)SDL_GL_GetProcAddress("glFogCoordPointer");
...
{
	gHUD.viewFrustum.SetFrustum(pparams->viewangles, pparams->vieworg, in_fov, gHUD.m_pFogSettings.end, true);
...
bool CBSPRenderer::DynamicLighted( const Vector &vmins, const Vector &vmaxs )
{
	...
	for(int i = 0; i < MAX_DYNLIGHTS; i++)
	{
		...
		if(m_pCurrentDynLight->cone_size)
		{
			if (m_pCurrentDynLight->frustum.CullBox((float*)Vector(vmins), (float*)Vector(vmaxs)))
				continue;
		}
		else
		{

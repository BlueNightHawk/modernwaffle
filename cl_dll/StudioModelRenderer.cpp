...
extern cvar_s* cl_righthand;

// Global engine <-> studio model rendering code interface
engine_studio_api_t IEngineStudio;

extern uint ScreenToScope;
...
float CStudioModelRenderer::StudioEstimateFrame(mstudioseqdesc_t* pseqdesc)
{
	...
	//			m_pCurrentEntity->curstate.animtime = m_clTime;
				//m_pCurrentEntity->latched.sequencetime = m_clTime;
...
void CStudioModelRenderer::StudioSetupBones(void)
{
	...
	if (gEngfuncs.GetViewModel() == m_pCurrentEntity)
	{
		...
		if (m_bNewViewModel)
		{
			...
			panim = StudioGetAnim(m_pRenderModel, pseqdesc);
			StudioCalcRotations(pos1b, q1b, pseqdesc, panim, 0);

			QuaternionMatrix(q1b[iBoneIndex], bonematrix);

			MatrixAngles(bonematrix, m_vBaseCamAngles);
		}
		QuaternionMatrix(q[iBoneIndex], bonematrix);

		MatrixAngles(bonematrix, gHUD.m_vCamAngles);
		gHUD.m_vCamAngles -= m_vBaseCamAngles;
	}
	...
	if (m_fDoInterp &&
		...
		(m_pCurrentEntity->latched.sequencetime + 0.16 > m_clTime) &&
		(m_pCurrentEntity->latched.prevsequence < m_pStudioHeader->numseq))
	{
		...
		s = 1.0 - (m_clTime - m_pCurrentEntity->latched.sequencetime) / 0.16;
...
int CStudioModelRenderer::StudioDrawModel(int flags)
{
	...
	if (m_pCurrentEntity == gEngfuncs.GetViewModel())
	{
		...
		if (pPrevModel != m_pRenderModel)
		{
			...
			{
				auto pDecal = reinterpret_cast<studiodecal_t*>(m_pCurrentEntity->efrag);
				studiodecal_t* pNext = nullptr;
				int total = pDecal->totaldecals;

				pDecal->totaldecals = 0;
	...
	if (m_pCurrentEntity == gEngfuncs.GetViewModel() && (int)cl_righthand->value > 0)
	{
...
void CStudioModelRenderer::StudioSetTextureFlags(void)
{
	...
	if (m_pStudioHeader->numtextures != NULL)
	{
		for (int i = 0; i < m_pTextureHeader->numtextures; i++)
		{
			...
			if (gTextureLoader.TextureHasFlag(szModel, szTexture, TEXFLAG_SCOPE) && !(ptexture[i].flags & STUDIO_NF_SCOPE))
			{
				ptexture[i].flags |= STUDIO_NF_SCOPE;
				bWriteFile = true;
			}
		}
	}
	...
	if (m_pStudioHeader->numtextures != NULL)
	{
		...
		for (int i = 0; i < pOpenHdr->numtextures; i++)
		{
			...
			if (gTextureLoader.TextureHasFlag(szModel, szTexture, TEXFLAG_SCOPE) && !(ptexturehdr[i].flags & STUDIO_NF_SCOPE))
				ptexturehdr[i].flags |= STUDIO_NF_SCOPE;
		}
	}
...
void CStudioModelRenderer::Mod_LoadTexture(mstudiotexture_t* ptexture, byte* pbuffer, char* szmodelname)
{
	...
	if (gTextureLoader.TextureHasFlag(szModelName, szTexture, TEXFLAG_SCOPE))
		ptexture->flags |= STUDIO_NF_SCOPE;
